...
outOfViewArrowsSize = 0,
outOfViewArrowsRadius = 0,
outOfViewArrowsColor = Color3.fromRGB(244, 95, 209),
outOfViewArrowsTransparency = 1,
outOfViewArrowsOutline = false,
outOfViewArrowsOutlineFilled = false,
outOfViewArrowsOutlineColor = Color3.fromRGB(0, 0, 0),
...
boxes = false,
boxesTransparency = 1,
boxesColor = Color3.fromRGB(244, 95, 209),
boxFill = false,
boxFillTransparency = 0.5,
boxFillColor = Color3.fromRGB(244, 95, 209),
healthBars = false,
healthBarsSize = 1,
...
healthTextSuffix = "%",
healthTextColor = Color3.fromRGB(244, 95, 209),
distance = false,
distanceTransparency = 1,
distanceSuffix = " Studs",
distanceColor = Color3.fromRGB(244, 95, 209),
tracers = false,
tracerTransparency = 1,
tracerColor = Color3.fromRGB(244, 95, 209),
tracerOrigin = "Bottom",
chams = false,
chamsFillColor = Color3.fromRGB(244, 95, 209),
...
if (not drawing) then
insert(espLib.instances, object);
end
return object;
end
...
function espLib.getBoundingBox(character, torso)
if (espLib.options.boundingBox) then
local minX, minY, minZ = inf, inf, inf;
local maxX, maxY, maxZ = -inf, -inf, -inf;
for _, part in next, espLib.options.boundingBoxDescending and getDescendants(character) or getChildren(character) do
if (isA(part, "BasePart") and not find(espLib.options.excludedPartNames, part.Name)) then
...
return torso.Position, vector2New(espLib.options.scaleFactorX, espLib.options.scaleFactorY);
end
end
function espLib.getScaleFactor(fov, depth)
if (fov ~= lastFov) then
...
function espLib.getBoxData(position, size)
local torsoPosition, onScreen, depth = worldToViewportPoint(position);
local scaleFactor = espLib.getScaleFactor(currentCamera.FieldOfView, depth);
local clampX = clamp(size.X, espLib.options.minScaleFactorX, espLib.options.maxScaleFactorX);
local clampY = clamp(size.Y, espLib.options.minScaleFactorY, espLib.options.maxScaleFactorY);
...
function espLib.visibleCheck(character, position)
local origin = currentCamera.CFrame.Position;
local params = raycastParamsNew();
params.FilterDescendantsInstances = { espLib.getCharacter(localPlayer), currentCamera, character };
params.FilterType = Enum.RaycastFilterType.Blacklist;
params.IgnoreWater = true;
return (not raycast(workspace, origin, position - origin, params));
end
function espLib.addEsp(player)
...
espLib.espCache[player] = objects;
end
function espLib.removeEsp(player)
local espCache = espLib.espCache[player];
if (espCache) then
espLib.espCache[player] = nil;
...
function espLib.addChams(player)
if (player == localPlayer) then
return
end
espLib.chamsCache[player] = create("Highlight", {
Parent = screenGui,
});
end
function espLib.removeChams(player)
local highlight = espLib.chamsCache[player];
if (highlight) then
espLib.chamsCache[player] = nil;
highlight:Destroy();
end
end
function espLib.addObject(object, options)
espLib.objectCache[object] = {
options = options,
text = create("Text", {
Center = true,
Size = 13,
...
function espLib.removeObject(object)
local cache = espLib.objectCache[object];
if (cache) then
espLib.objectCache[object] = nil;
cache.text:Remove();
end
end
function espLib:AddObjectEsp(object, defaultOptions)
...
function espLib:Load(renderValue)
